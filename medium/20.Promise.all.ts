...
/**
 * Description:
 * MyAwaited 是我们之前自己实现过的代码，可以提取 Promise 中的类型
 * PromiseAll 的目的是，遍历 values 中的每一个元素，如果是 PromiseLike 类型，就提取出来，如果不是，就直接返回。
 * 比较 tricky 的是 `P in keyof T` 这一行，这一行尽管看起来像是将 tuple 变成了一个 object，但实际上这是一个 mapping 操作，针对 T 中的每一个元素都进行了一个特定操作。
 */